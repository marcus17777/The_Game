    ...
    def generate_map_chunk(self, xy):
        ...
        for _y in range(self.world_map_height):
            ...
            for _x in range(self.world_map_width):
                temp_map[-1].append('')
                self.tasks.put(MapGenerator((_x, _y),
                                            (_x + xy[0] * self.world_map_width) / self.world_map_frequency,
        ...
        while not self.results.empty():
            ...
            if value >= 0.9 * 255:
                ...
                value = 0
            temp_map[block[1][1]][block[1][0]] = value
            # f.write("%s" % block)
            # f.write('\n')
        # f.close()
...
class MapGenerator(object):
    ...
    def __call__(self):
        return (
            round(
                noise.snoise3(self.x, self.y, self.x, self.octaves, self.persistence, self.lacunarity) * 127.0 + 128.0),
            self.position)
